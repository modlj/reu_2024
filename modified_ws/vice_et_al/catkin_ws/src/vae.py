...
# Designed for training and evaluating a VAE model for image data.
#It includes components for data preprocessing, model architecture,
...
# Used to perform sampling in the VAE. Sampling is used to generate 
#new data points from the learned latent space
#	1. sampling_model takes distribution parameters (mean and long 
#      variance) and generates samples
#	2. sampling constructs a model that takes mean and long variance 
#      as inputs and produces samples using the sampling_model
...
# encoder, decoder, vae_loss functions
#	1. encoder defines a convolutional neural network that encodes input data 
...
#	2. decoder defines a network that decodes latent representations 
...
#	3. Various loss functions defined for training the VAE
...
# Used for a single training step. It computes the loss and applies gradients 
...
# Trains the VAE for a specified number of epochs. It iterates over the dataset 
#and calls train_step in each iteration
...
# Generates and saves reconstructed images after each epoch of training.
#	1.Takes the trained encoder, final sampling layer, and decoder 
#     models and uses them to generate images from latent representations
...
# Configures the training process. It defines model parameters, loads datasets, 
#and trains the VAE for a specified number of epochs
...
#  Converts an RGB image to grayscale using a specified formula
def rgb2gray(rgb):
    return np.dot(rgb[...,:3], [0.2989, 0.5870, 0.1140])

# Displays and plots image predictions
...
# Serves as primary training and evaluation loop:
#	1. Continuously dequeues data from the queue, computes structural 
#      similarity (SSIM) between the original and generated images, 
#      and publishes SSIM values
#	2. Handles model saving and logging SSIM values  
...
# Class subscribes to ROS image messages, converts them to OpenCV images, 
#resizes and converts them to grayscale, and enqueues them in the queue q
...
# Creates a First-In, First-Out (FIFO) queue with a specified size
def build_q():
    q_size = 100 #20
    shape=(256,256,1)
...
# Class defines configuration parameters such as batch size, epochs, 
#and the model path
class Config:
...
# encoder_model and decoder_model define the architecture of the encoder 
#and decoder models used in the VAE. Layer normalization is applied in 
#various parts of the models  
#	1. Encoder takes sequences of images as input and encodes them into 
#      a latent space 
...
#	2.Takes sequences of latent representations and decodes them into sequences of images 
...
# Generator functions yield batches of data for training and evaluation. 
#They dequeue data from the queue and reshape it as needed
...
# Creates and compiles the VAE model and an encoder-only model for inference
...
# Initializes the ROS node, builds the queue, and calls the evaluate 
#function, which starts the main training and evaluation loop    
