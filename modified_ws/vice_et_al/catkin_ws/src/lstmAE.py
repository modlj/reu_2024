...
# Designed to run a ConvLSTM-based exploration node within a ROS environment, 
#continuously processing image data, and using ConvLSTM #models to calculate 
#SSIM values and make decisions based on them
...
# For performing inference using the trained model:
#	1. Subscribes to an image topic from ROS, runs inference on recieved images 
#	2. Publishes SSIM values and reconstructed images to ROS topics
...
# Responisble for subscribing to image data from ROS and running inference
...
# 1. Loads or initializes the ConvLSTM model
# 2. Dequeues a batch of images from the queue q
# 3. Feeds images into model and calculates the structural siimilarity
#    between the input and reconstructed images
# 4. Saves model if certain SSIM thresholds are met
# 5. Publishes SSIM values to a ROS topic
# 6. Maintains a moving average of SSIM values
# 7. Continues until interrupted  
def exec_main_loop(model, model_inf, q, pubS):
    ...
    reconFile = open("/home/johnmodl/catkin_ws/src/csvICRA/SSIM-1dot5e-4"+ timestr + ".csv","w")
...
# Sets up and executes the main loop for model evaluation:
#	1. Loads or initializes the model
#	2. Dequeues image data
#	3. Performs inference
#	4. Calculates SSIM
#	5. Maintains a moving average of SSIM values 
def evaluate(q):
...
# Subscribes to a ROS image topic, converts the received images to #grayscale, 
#resizes them, and enqueues them into the queue q for further processing
...
# Utility function for creating a FIFO queue
def build_q():
    q_size = 100 #20
    shape=(256,256,1)
...
# Configuration constants: batch size, number of epochs, and path to save
#the model 
class Config:
    BATCH_SIZE = 1 # 4 was original
    EPOCHS = 1 # change back to 3 (Jack)
    MODEL_PATH = "/home/johnmodl/catkin_ws/src/video-anomaly-detection-master/notebooks/lstmautoencoder/model.hdf5"

# Defines encoder_model and decoder_model to create the ConvLSTM model's
#architecture, both for training and inference   
...
# ..._queue and ..._queue_test create data generators to feed image 
#sequences to the model during training and testing
...
# Initializes and compiles the ConvLSTM model and encoder-only 
